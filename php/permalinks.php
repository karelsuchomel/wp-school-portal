 ...
 */
class SchoolPortal_SetPermalinks {
	/**
	 * Set up actions
	 */
	...
	public function admin_permalinks_warning() {
		...
		<div class="notice notice-warning">
			<p><?php _e( '<b>Warning:</b> The theme you\'re using does not support customized permalinks. See the Help tab above for more information.', 'school_portal' ); ?></p>
		</div>
		<?php
	}
	...
	public function admin_theme_warning() {
		?>
		<div class="notice notice-warning">
			<p><?php _e( 'This theme requires special URLs to display your content, so your permalinks have been updated. To undo this, switch to another theme.', 'school_portal' ); ?></p>
		</div>
		<?php
	}
	...
	public function add_contextual_permalink_help() {
		...
		$content = '<p>' . __( 'School portal is a different kind of theme, which means it needs to handle URLs differently than other themes. There is no permalink customization in school portal, you need to use the "Month and name" setting for posts.', 'school_portal' ) . '</p>';
		$content .= '<p>' . __( 'Other content on your site also needs different URLs, like the following:', 'school_portal' ) . '<ul>';
		$content .= '<li>' . __( 'Pages are now prefixed with <code>page</code>; for example, <code>%1$s/page/about/</code>', 'school_portal' ) . '</li>';
		$content .= '<li>' . __( 'Date archives are now prefixed with <code>date</code>; for example, <code>%1$s/date/2016/12/</code>', 'school_portal' ) . '</li>';
		$content .= '<li>' . __( 'Category and tag archives are prefixed with <code>category</code> and <code>tag</code>, which is core behavior, but this cannot be changed.', 'school_portal' ) . '</li>';
		$content .= '</ul></p>';

		$screen->add_help_tab( array(
			'id'      => 'school_portal_permalinks_help',
			'title'   => __( 'School portal Permalinks', 'school_portal' ),
...
new SchoolPortal_SetPermalinks();
